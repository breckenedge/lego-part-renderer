...
func TestRenderGoldenFiles(t *testing.T) {
	...
	params := map[string]partParams{
		...
		"3024-plate-1x1":                     {2.0, "currentColor", 0, "cyan"},
		...
		"4740-dish-2x2-inverted":             {1.0, "#9b59b6", 0.5, ""},
	...
	for _, entry := range entries {
		...
		t.Run(fileKey, func(t *testing.T) {
			...
			body, _ := json.Marshal(RenderRequest{PartNumber: partNumber, Thickness: p.thickness, FillColor: p.fillColor, FillOpacity: &fillOpacity, StrokeColor: p.strokeColor})
